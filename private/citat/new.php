...
if(!isset($_SESSION["a"])){
  header("location: " . $pathAPP . "logout.php");
	...
	if(isset($_POST["dodaj"])){
		$query = $connect->prepare("insert into citat (korisnik,citat) values
		    (:korisnik,:citat)");
	  	unset($_POST["dodaj"]);
	  	$query->execute($_POST);
							...
							<header class="align-center">
								<form method="POST" action="<?php echo $_SERVER["PHP_SELF"] ?>">
									...
									<select name="korisnik" id="korisnik">
										...
										<option value="<?php echo $row->sifra; ?>"><?php echo $row->ime . ' ' . $row->prezime; ?></option>

										<?php endforeach; ?>
									</select>
									<label for="citat">Citat</label>
									<textarea name="citat" id="citat" cols="30" rows="10"></textarea>
