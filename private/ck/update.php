<?php include_once "../../config.php"; 
	...
	if(isset($_POST["promjeni"])){
		$query = $connect->prepare("update citatkategorija set citat=:citat,kategorija=:kategorija where sifra=:sifra");
	  	unset($_POST["promjeni"]);
	  	$query->execute($_POST);
	  	header("location: index.php");
	}else {
	  $query = $connect->prepare("select * from citatkategorija where sifra=:sifra");
	  $query->execute($_GET);
								...
								<form method="POST" action="<?php echo $_SERVER["PHP_SELF"] ?>">
									...
									<select name="citat" id="citat">
										...
										<?php 
					                      ...
					                      if(isset($_POST["citat"]) && $_POST["citat"]==$row->sifra){
					                        echo ' selected="selected" ';
					                      }

					                    ?>
										value="<?php echo $row->sifra; ?>"><?php echo $row->citat; ?></option>
